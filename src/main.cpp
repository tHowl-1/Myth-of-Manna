...
int main(int argc, char* argv[]) {
    ...
    while (true) {  // Game loop.
        ...
        //simulation.update(); // Update and render doom fire

        // Render player as "@" character to console in white (255, 255, 255) foreground color and with black (0, 0, 0) background color
        tcod::print(console, { player_x, player_y }, "@", { {255, 255, 255} }, { {0, 0, 0} });

        context->present(console);  // Updates the visible display

        SDL_Event event;
        SDL_WaitEvent(nullptr); //Optional, sleep until events are available WARNING This will cause pausing in doom fire simulation!
        while (SDL_PollEvent(&event)) {
            context->convert_event_coordinates(event);  // Optional, converts pixel coordinates into tile coordinates
            switch (event.type) {
            case SDL_KEYDOWN:   // TODO Move input handling to separate class
                switch (event.key.keysym.sym) // Check key type
                {
                case SDLK_UP:
                    player_y--; // TODO Return action class instead of directly modifying values
                    break;
                case SDLK_DOWN:
                    player_y++;
                    break;
